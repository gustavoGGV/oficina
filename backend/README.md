...
| Tema | Problema deliberado | Como observar | Correção implementada pelo modo corrigido |
| --- | --- | --- | --- |
| 1 · Margem | **SQL Injection no login** | Uma UNION altera o ID e o hash retornados pela consulta; é possível entrar como admin usando o hash de uma conta cliente. | Consulta preparada com parâmetro para o e-mail, seguida de verificação de senha. |
| 2 · RUÍDO | **Falta de autorização** | Ana e Bruno conseguem abrir `admin.php`, embora sejam clientes. | Exigir `role === 'admin'` no servidor antes de emitir o painel. |
| 3 · PULSO | **Token sem verificação de assinatura / autenticação fraca** | Alterar `sub` de 1 para 3 no JWT faz o servidor reconhecer a conta admin. | Verificar a assinatura HMAC, além de algoritmo, destinatário, emissor e expiração. |
| 4 · Vereda | **XSS refletido no erro de login** | O e-mail enviado volta na mensagem e é inserido como HTML, permitindo executar um marcador visual. | Mensagem genérica e exibição com `textContent`, sem interpretar entrada como HTML. |
| 5 · Grão | **Chave secreta exposta no frontend** | O script `auth.php?action=lab-key` revela a chave HS256 do tema; ela permite assinar um token adulterado. | Deixar a chave exclusivamente no servidor e rotacionar a chave já exposta. O modo corrigido para de entregá-la; execute também o reset para invalidar assinaturas antigas. |
