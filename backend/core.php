...
function lab_vulnerable(int $site): bool {
    return !lab_config()['fixed'];
}
...
function lab_identity(int $site, ?string $token = null): ?array {
    ...
    try {
        ...
        // FALHA 3 — PULSO: lê a identidade sem conferir a assinatura do JWT.
        if (!($site === 3 && lab_vulnerable($site))) {
...
function lab_login(int $site, string $email, string $password): ?array {
    ...
    if ($site === 1 && lab_vulnerable($site)) {
        // FALHA 1 — MARGEM: concatenação permite manipular o resultado da consulta.
        $row = lab_db()->query("SELECT id, password_hash FROM users WHERE email = '$email' LIMIT 1")->fetch();
    } else {
...
function lab_can_admin(int $site, ?array $user): bool {
    ...
    // FALHA 2 — RUÍDO: qualquer cliente autenticado pode abrir a área administrativa.
    return ($site === 2 && lab_vulnerable($site)) || $user['role'] === 'admin';
}
