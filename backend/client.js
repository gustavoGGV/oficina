/* Login compartilhado. Cada auth.php fixa o tema no servidor.
   Somente o site 4 ativa deliberadamente a inserção insegura de HTML. */
(() => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        // FALHA 4 — XSS refletido. No modo corrigido, a resposta usa textContent.
        if (data.unsafe_error_html) feedback.innerHTML = data.message;
        else feedback.textContent = data.message || "Não foi possível entrar.";
        feedback.focus();
        return;
      }
      form.reset();
