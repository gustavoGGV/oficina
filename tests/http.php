...
foreach (range(1, 5) as $site) {
  ...
  expect(request("$path/admin.php", null, $cookie)['code'] === ($site === 2 && !$fixed ? 200 : 403), "$site: autorização de cliente");
  ...
  expect(request("$path/admin.php", null, $tampered)['code'] === ($site === 3 && !$fixed ? 200 : 302), "$site: token adulterado");
  $key = request("$path/auth.php?action=lab-key");
  if ($site === 5 && !$fixed) {
    preg_match('/= "([a-f0-9]+)";/', $key['body'], $m);
    expect(isset($m[1]), '5: chave exposta na resposta pública');
    $signature = b64(hash_hmac('sha256', "$h.$p", $m[1], true));
    expect(request("$path/admin.php", null, "oficina_5=$h.$p.$signature")['code'] === 200, '5: chave exposta permite forjar admin');
  } else expect(str_contains($key['body'], '= null;'), "$site: chave não exposta");
  ...
  expect($attack['code'] === ($site === 1 && !$fixed ? 200 : 401), "$site: SQL injection delimitada");
  if ($site === 1 && !$fixed) expect($attack['json']['user']['role'] === 'admin', '1: SQLi obtém identidade admin');
  $xss = '<img src=x onerror="alert(1)">';
  $error = request("$path/auth.php", ['action' => 'login', 'email' => $xss, 'password' => 'invalida']);
  expect($error['code'] === 401, "$site: senha inválida rejeitada");
  expect(($error['json']['unsafe_error_html'] ?? false) === ($site === 4 && !$fixed), "$site: XSS delimitado");
  if ($site === 4 && !$fixed) expect(str_contains($error['json']['message'], $xss), '4: entrada refletida no erro');
  else expect(!str_contains($error['json']['message'], $xss), "$site: mensagem sem reflexão de HTML");
